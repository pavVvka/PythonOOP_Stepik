...
select(self, a, b) - возвращает список из элементов списка lst_data
в диапазоне [a; b] (включительно) по их индексам (не id, а индексам списка);
также учесть, что граница b может превышать длину списка.

Каждая запись в списке lst_data должна быть представлена словарем
(добавление с помощью метода insert) в формате:
...
class DataBase:
    ...
    def insert(self, data):
        for string in data:
            self.lst_data.append(dict(zip(self.FIELDS, string.split())))

    def select(self, a, b):
        if len(self.lst_data) >= b:
            print("sp >= b : ", self.lst_data[a:b+1])
            return self.lst_data[a:b+1]
        print("sp < b : ", self.lst_data[a:])
