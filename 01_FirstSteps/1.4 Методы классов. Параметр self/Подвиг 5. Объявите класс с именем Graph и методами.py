...
Метод set_data() должен формировать
локальное свойство data объекта класса Graph.
...
разделенных пробелами и принадлежащие заданному диапазону
атрибута LIMIT_Y (границы включаются).

Создайте объект graph_1 класса Graph,
вызовите для него метод set_data() и передайте список:
...
class Graph:
    ...
    def draw(self):
        tmp = (x for x in self.data if Graph.LIMIT_Y[0] <= x <= Graph.LIMIT_Y[1])
        print(*tmp)
        # var.2
        a, b = self.LIMIT_Y
        print(*(filter(lambda x: a <= x <= b, self.data)))
        # var.3
        print(" ".join(map(str, filter(lambda x: a <= x <= b, self.data))))
